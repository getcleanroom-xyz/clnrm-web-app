...
export function StreamPlayer({ sessionId, token }: StreamPlayerProps) {
  ...
  // Ready - show noVNC iframe
  if (isReady) {
    const vncUrl = token
      ? `/api/stream/${sessionId}?token=${encodeURIComponent(token)}`
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-green/12 bg-surface/80 backdrop-blur-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-green font-bold tracking-[0.1em] uppercase">Connected</span>
            </div>
          </div>
